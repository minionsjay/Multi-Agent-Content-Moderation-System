...
async def text_specialist(state: ModerationState) -> dict:
    ...
    if bert_enabled:
        ...
        # Determine if this model can handle the text's language
        model_is_english_only = (
            "toxic-bert" in bert_model_name.lower()
            and "xlm" not in bert_model_name.lower()
            and "multilingual" not in bert_model_name.lower()
            and "koala" not in bert_model_name.lower()
        )
        if is_chinese and model_is_english_only:
    ...
    # Select LLM backend: API (DeepSeek/OpenAI/Anthropic) or local (llama.cpp)
