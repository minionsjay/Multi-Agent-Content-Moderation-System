#!/usr/bin/env python3
"""Benchmark using tdavidson/hate_speech_offensive."""
...
def load_hatespeech(n: int | None = None, seed: int = 42) -> list[dict]:
    from datasets import load_dataset

    ds = load_dataset("tdavidson/hate_speech_offensive", split="train")
    print(f"Loaded {len(ds)} samples from hate_speech_offensive")

    cases = []
    for row in ds:
        # class 0=hate_speech, 1=offensive_language, 2=neither
        label = "unsafe" if row["class"] in (0, 1) else "safe"
        cases.append({"text": row["tweet"], "expected": label, "orig_class": row["class"]})
    ...
    cc = Counter(c["orig_class"] for c in sampled)
    cls_names = {0: "hate_speech", 1: "offensive_language", 2: "neither"}
    for k in sorted(cc.keys()):
        print(f"  {cls_names.get(k, k)}: {cc[k]} ({cc[k]/len(sampled)*100:.1f}%)")
...
async def run_one(text: str, content_id: str) -> dict:
    ...
    state: ModerationState = {
        ...
        "user_id": "benchmark", "source": "hatespeech", "content_type": "text_only",
...
def compute_metrics(cases: list[dict], results: list[dict]) -> dict:
    ...
    cls_names = {0: "hate_speech", 1: "offensive_language", 2: "neither"}
    class_stats = {v: {"tp": 0, "fp": 0, "fn": 0, "tn": 0, "total": 0} for v in cls_names.values()}
    ...
    for case, res in zip(cases, results):
        ...
        if expected == "unsafe" and predicted in ("block", "review"):
            tp += 1
            class_stats[cls_names[case["orig_class"]]]["tp"] += 1
        elif expected == "safe" and predicted == "pass":
            tn += 1
            class_stats[cls_names[case["orig_class"]]]["tn"] += 1
        elif expected == "unsafe" and predicted == "pass":
            fn += 1
            class_stats[cls_names[case["orig_class"]]]["fn"] += 1
        elif expected == "safe" and predicted in ("block", "review"):
            fp += 1
            class_stats[cls_names[case["orig_class"]]]["fp"] += 1

        class_stats[cls_names[case["orig_class"]]]["total"] += 1
    ...
    for cls, s in class_stats.items():
        if s["total"] > 0:
            per_class[cls] = {
                "total": s["total"],
                "accuracy": round((s["tp"] + s["tn"]) / s["total"], 4),
                "recall": round(s["tp"] / (s["tp"] + s["fn"]), 4) if (s["tp"] + s["fn"]) > 0 else 0,
...
def print_report(metrics: dict, elapsed_s: float):
    print()
    print("=" * 60)
    print("  POC Benchmark — tdavidson/hate_speech_offensive")
    print("=" * 60)
    ...
    if metrics.get("per_class"):
        ...
        for cls, s in metrics["per_class"].items():
            print(f"    {cls:20s}: acc={s['accuracy']:.1%} rec={s['recall']:.1%} "
                  f"(tp={s['tp']} fn={s['fn']} n={s['total']})")
    print()
    checks = [
...
async def main():
    ...
    print(f"Hate Speech Benchmark: n={args.n} concurrency={args.concurrency}")
    cases = load_hatespeech(n=args.n)

    sem = asyncio.Semaphore(args.concurrency)

    async def process_one(i: int, case: dict) -> dict:
        async with sem:
            res = await run_one(case["text"], f"hs_{i}")
            res["text"] = case["text"]
            res["expected"] = case["expected"]
            res["orig_class"] = case["orig_class"]
            return res
    ...
    shown = 0
    cls_names = {0: "hate_speech", 1: "offensive", 2: "neither"}
    for r in results:
        ...
        if not correct and shown < 6:
            orig = r.get("orig_class", "?")
            print(f"\n  Text: {r.get('text', '')[:150]}")
            print(f"  Orig class: {cls_names.get(orig, orig)} | Expected: {expected} | "
                  f"Predicted: {predicted} | Conf: {r.get('confidence', 0):.2f} | "
                  f"Tier: {r.get('tier', '?')}")
